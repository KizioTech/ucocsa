...
const Dashboard = () => {
  ...
  return (
    <Layout>
      <section className="py-24 bg-gradient-to-tr from-muted/30 to-background min-h-screen relative overflow-hidden">
        ...
        <div className="container max-w-6xl mx-auto px-4 relative z-10 space-y-12">
          ...
          <motion.div 
             ...
             className="flex flex-col md:flex-row items-center justify-between gap-8 pb-12 border-b border-border"
          >
            <div className="flex items-center gap-6">
              ...
              <div>
                 <h1 className="text-3xl md:text-5xl font-heading text-foreground">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button variant="outline" asChild className="rounded-2xl border-border text-foreground hover:bg-muted shadow-sm h-10 px-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              >
                <Link to={stat.link}>
                  <Card className="bg-card shadow-sm backdrop-blur-xl border-border hover:border-primary/40 transition-all hover:shadow-2xl hover:shadow-primary/5 group relative overflow-hidden py-4">
                    ...
                    <CardContent>
                      <p className="text-xs uppercase font-bold text-muted-foreground tracking-widest">{stat.label}</p>
                    </CardContent>
                  </Card>
                </Link>
              </motion.div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
             ...
             <div className="lg:col-span-2 space-y-8">
                {/* Events */}
                <Card className="bg-card shadow-sm backdrop-blur-md border-border overflow-hidden rounded-[2rem]">
                   <CardHeader className="p-6 md:p-8 border-b border-border flex flex-row items-center justify-between">
                    ...
                    <div className="divide-y divide-white/5">
                      {upcomingEvents.length === 0 ? (
                        ...
                        upcomingEvents.map((event) => (
                          <div key={event.id} className="p-4 md:p-6 flex items-center gap-4 md:gap-6 group hover:bg-muted/50 transition-colors">
                            ...
                            <div className="flex-1 min-w-0">
                               <p className="font-heading text-base md:text-lg text-foreground group-hover:text-primary transition-colors truncate">{event.title}</p>
                               <p className="text-sm text-muted-foreground flex items-center gap-2 mt-1">
                                 <Clock size={12} /> {event.event_time || "TBA"} • <MapPin size={12} /> {event.location || "On Campus"}
                              </p>
                            </div>
                            <Badge variant="secondary" className="bg-muted text-foreground tracking-tighter border-0 hidden sm:inline-flex">{event.event_type}</Badge>
                          </div>
                        ))
                      )}
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                   <div className="md:col-span-3 flex items-center justify-between mb-2">
                        <h3 className="text-xl font-heading text-foreground">From the Journal</h3>
                        <Link to="/blog" className="text-xs font-bold text-primary hover:underline uppercase tracking-widest">Read More</Link>
                   </div>
                   {recentBlogPosts.map((post) => (
                      <Link key={post.id} to={`/blog/${post.slug}`}>
                         <Card className="bg-card shadow-sm border-border hover:border-primary/20 transition-all group overflow-hidden h-full rounded-2xl">
                            ...
                            <CardContent className="p-4">
                               ...
                               <h4 className="text-foreground text-sm font-bold line-clamp-2 leading-tight">{post.title}</h4>
                            </CardContent>
                         </Card>
                      </Link>
                   ))}
             ...
             <div className="space-y-8">
                {/* Announcements */}
                 <Card className="bg-card shadow-sm border-border rounded-[2rem] overflow-hidden">
                   ...
                   <CardContent className="p-6 space-y-6">
                      {announcements.map((ann) => (
                         <div key={ann.id} className="relative pl-6 border-l border-primary/30 group">
                            ...
                            <h4 className="text-foreground font-bold text-sm mb-1 group-hover:text-primary transition-colors truncate">{ann.title}</h4>
                            <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">{ann.content}</p>
                            <p className="text-[10px] text-muted-foreground mt-2 font-bold uppercase tracking-widest">{format(new Date(ann.published_at), "MMM d, yyyy")}</p>
                        </div>
                      ))}
                   </CardContent>
                </Card>
                ...
                {/* Prayer List */}
                 <Card className="bg-card shadow-sm backdrop-blur-md border-border rounded-[2rem] overflow-hidden">
                   <CardHeader className="p-6 md:p-8 border-b border-border">
                     <CardTitle className="text-xl text-foreground flex items-center gap-2 font-heading">
                  ...
                  <CardContent className="p-6 space-y-4">
                    {myPrayers.slice(0, 3).map((prayer) => (
                       <div key={prayer.id} className="p-4 rounded-2xl bg-muted/50 border border-border hover:border-primary/20 transition-all">
                         <p className="text-muted-foreground text-xs italic line-clamp-2 leading-relaxed">"{prayer.request}"</p>
                        <div className="flex items-center justify-between mt-4">
                           <Badge variant="outline" className="text-[9px] uppercase border-border text-muted-foreground tracking-tighter">
                              {prayer.status}
                           </Badge>
                    ...
                    <Button variant="ghost" className="w-full text-xs text-muted-foreground hover:text-foreground uppercase tracking-[0.2em] font-bold" asChild>
