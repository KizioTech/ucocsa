  ...
  <Layout>
    <section className="py-20 bg-secondary">
      <div className="container text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-4xl md:text-5xl font-heading text-secondary-foreground">Give</h1>
          <p className="mt-4 text-secondary-foreground/70 max-w-lg mx-auto">
            Your generosity fuels the mission. Support UCOCSA's work on campus and beyond.
          </p>
        </motion.div>
      </div>
    </section>
    ...
    <section className="py-16">
      <div className="container max-w-4xl">
        <SectionHeading title="Ways to Give" subtitle="Choose a giving option below." />
        <div className="grid sm:grid-cols-3 gap-6">
          {[
            { title: "One-Time Gift", desc: "Any amount to support UCOCSA activities", icon: Heart },
            { title: "Association Dues", desc: "Annual membership dues (MK 2,000)", icon: CreditCard },
            { title: "Tithe & Offering", desc: "Regular giving to the ministry", icon: Heart },
          ].map((option, i) => (
            <motion.div
              key={option.title}
        ...
        >
          <h3 className="font-heading text-lg text-secondary-foreground mb-3">Mobile Money</h3>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center text-sm">
            <div className="px-6 py-3 rounded-lg bg-secondary-foreground/10">
              <span className="font-medium text-secondary-foreground">Airtel Money:</span>{" "}
              <span className="text-secondary-foreground/80">0999 000 000</span>
            </div>
            <div className="px-6 py-3 rounded-lg bg-secondary-foreground/10">
              <span className="font-medium text-secondary-foreground">TNM Mpamba:</span>{" "}
              <span className="text-secondary-foreground/80">0888 000 000</span>
            </div>
          </div>
          <p className="mt-3 text-xs text-secondary-foreground/50">Reference: UCOCSA + Your Name</p>
        </motion.div>
      </div>
    </section>
  </Layout>
